export default async function ({ addon, global, console, safeMsg }) {
  ...
  const addEmbedFunction = function () {
    ...
    if (
      global.scratchProjectRegExp.test(url.hostname + url.pathname) ||
      global.youTubeRegExp.test(url.hostname + url.pathname + url.search) ||
      global.youTubeDiscussRegExp.test(url.hostname + url.pathname) ||
      global.audioRegExp.test(url.pathname) ||
      global.videoRegExp.test(url.pathname)
    ) {
