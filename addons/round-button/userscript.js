    ...
    for (const item of selectedItems) {
      ...
      const curvesToRound =
        selectedSegments.length > 0
          ? Array.from(new Set(selectedSegments.map((s) => s.curve)))
  ...
  const updateButtonDisabled = () => {
    if (roundButton) {
      if (addon.tab.redux.state.scratchPaint.selectedItems.length > 0) {
        ...
        roundButton.classList.remove(addon.tab.scratchClass("button_mod-disabled"));
      } else {
        roundButton.setAttribute("disabled", "");
        roundButton.classList.add(addon.tab.scratchClass("button_mod-disabled"));
      }
    }
  };
      ...
      '[class*="mode-tools_mod-dashed-border_"]:first-child > span:first-child',
      {
        ...
        reduxCondition: (state) => {
          return (
            ...
            (state.scratchPaint?.mode === "RESHAPE" || state.scratchPaint?.mode === "SELECT")
          );
        },
      }
    );
